...
TELEGRAM_BOT_TOKEN = "7820597214:AAGzuNDwDd5cErhKRFv-3wV648nkBjF0kjA"
TELEGRAM_CHAT_ID = "5408418693"

# MongoDB configuration
MONGO_URI = "mongodb+srv://pradyumnchauhan:Pr%40d1234@cluster0.a0fu5.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
DATABASE_NAME = "BTC"
...
def send_telegram_status(message):
    ...
    try:
        ...
        print(f"Error sending Telegram status: {e}")
...
def fetch_tickers():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            send_telegram_status(f"✅ Tickers API: Successfully fetched {len(btc_tickers)} BTC tickers")
            return btc_tickers
        else:
            error_msg = f"❌ Tickers API Error: Status code {response.status_code}"
            send_telegram_status(error_msg)
            print(error_msg)
            return None
    except Exception as e:
        error_msg = f"❌ Tickers API: Failed to fetch data - {str(e)}"
        send_telegram_status(error_msg)
        print(error_msg)
        return None
...
def fetch_oi_change():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            send_telegram_status("✅ OI Change API: Data fetched successfully")
            return result
        else:
            error_msg = f"❌ OI Change API Error: Status code {response.status_code}"
            send_telegram_status(error_msg)
            print(error_msg)
            return None
    except Exception as e:
        error_msg = f"❌ OI Change API: Failed to fetch data - {str(e)}"
        send_telegram_status(error_msg)
        print(error_msg)
        return None
...
def fetch_taker_volume(start_time, end_time):
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            result = data.get("result", {})
            send_telegram_status(f"✅ Taker Volume API: Data fetched successfully\n"
                               f"Time window: {datetime.datetime.fromtimestamp(start_time).strftime('%H:%M:%S')} - "
                               f"{datetime.datetime.fromtimestamp(end_time).strftime('%H:%M:%S')}")
            return result
        else:
            error_msg = f"❌ Taker Volume API Error: Status code {response.status_code}"
            send_telegram_status(error_msg)
            print(error_msg)
            return None
    except Exception as e:
        error_msg = f"❌ Taker Volume API: Failed to fetch data - {str(e)}"
        send_telegram_status(error_msg)
        print(error_msg)
        return None
# ------------------------------
...
if __name__ == "__main__":
    while True:
        try:
            main()
            time.sleep(15)  # Wait for 15 seconds before next execution
        except Exception as e:
            print(f"Error in main loop: {e}")
            time.sleep(15)  # Still wait 15 seconds even if there's an error
